...
public class Philosopher extends Thread {
	...
	private int ID;
	private Semaphore leftSide;
	final int timeThink_max = 5000;
	final int timeNextFork = 100;
	final int timeEat_max = 5000;
	
	Philosopher(int ID, GraphicTable table, Chopstick left, Chopstick right, Semaphore leftSide) {
		this.ID = ID;
		this.table = table;
		this.left = left;
		this.right = right;
	...
	public void run() {
		while(true){
			...
			System.out.println(leftSide.availablePermits());
			left.take();
