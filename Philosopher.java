public class Philosopher extends Thread{
	...
	public void run() {
		while(true){
			...
			synchronized (left){
			while (left.take() == false){
				try{
				System.out.println("Chopstick is in use, waiting for philospher to finish with left chopstick");
				left.wait();
				}catch (Exception ex){
					ex.printStackTrace();
				}
			...
			synchronized (right){
				while (right.take() == false){
					try{
					System.out.println("Chopstick is in use, waiting for philospher to finish with right chopstick");
					right.wait();
					}catch (Exception ex){
						ex.printStackTrace();
					}
